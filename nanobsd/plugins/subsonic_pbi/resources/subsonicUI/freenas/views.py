...
def start(request):
    ...
    try:
        form = forms.SubsonicForm(subsonic.__dict__, instance=subsonic, jail=jail)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': 'Subsonic data did not validate, please configure it first.',
...
def treemenu(request):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('subsonic_edit'),
        'kwargs': {'plugin_name': 'subsonic'},
    }
...
def status(request):
    ...
    proc = Popen(["/usr/bin/pgrep", "-U", "dlna", "-F", "/var/run/subsonic.pid"], stdout=PIPE, stderr=PIPE)
